...
function show_workspace_options {
	...
	if [[ $FAILED == 0 ]]; then
		existingworkspaces=$(ws_list | perl -e 'my %struct = (); my $current_id = q##; while (<>) { if(m#^id: (.*)$#) { $current_id = $1 } elsif (m#remaining time\s*:\s*(.*)?$#) { $struct{$current_id}{remaining} = $1 } elsif (m#^\s*available extensions\s*:\s*(.*?)$#) { $struct{$current_id}{extensions} = $1; } }; foreach my $key (keys %struct) { print qq#"$key" "$struct{$key}{remaining} ($struct{$key}{extensions) extensions left))" OFF # }')
