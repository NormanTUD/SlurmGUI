    ...
    if command -v squeue &> /dev/null; then
        if command -v whiptail &> /dev/null; then
            ...
            if [[ $chosenjob == 'e)' ]]; then
                ...
                whattodo=$(eval "whiptail --title 'Slurminator $chosenjob' --menu 'Test' 16 100 9 's)' 'Show log path' 'w)' 'whypending' 'f)' 'tail -f' 'c)' 'scancel' 's)' 'scancel with signal USR1' 'e)' 'Exit program'" 3>&2 2>&1 1>&3)
                case $whattodo in
                    "s)")
