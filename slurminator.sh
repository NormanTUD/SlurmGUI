...
function slurminator {
    ...
    if [[ $chosenjob == 'e)' ]]; then
        ...
        whattodo=$(eval "whiptail --title 'Slurminator $chosenjob' --menu 'Test' 16 100 9 's)' 'Show log path' 'f)' 'tail -f' 'c)' 'scancel' 's)' 'secure cancel (USR1)' 'e)' 'Exit program'" 3>&2 2>&1 1>&3)
        case $whattodo in
            "s)")
